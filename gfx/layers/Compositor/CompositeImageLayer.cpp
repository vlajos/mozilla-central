...
CompositeImageLayer::EnsureImageHost(BufferType aHostType)
{
  if (!mImageHost ||
    ...
    RefPtr<BufferHost> bufferHost = mManager->GetCompositor()->CreateBufferHost(aHostType);
...
void
CompositeImageLayer::RenderLayer(const nsIntPoint& aOffset, const nsIntRect& aClipRect, Surface*)
{
  ...
  CompositeLayerManager::ToMatrix4x4(GetEffectiveTransform(), transform);
